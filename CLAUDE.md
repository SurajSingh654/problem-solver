...
The merged `masteryCounts` object passed to `classifyReadiness` aggregates D1, D2, D3 keys into one bag with **non-overlapping prefixes**: `core*` / `owned` / `solidOrAbove` / `workingOrAbove` (patterns), `solutionsAt*` (solutions), `comm*` (communication). When adding a new dimension's tier gates, pick a prefix that won't collide.
