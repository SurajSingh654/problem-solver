...
**Knowledge Retention v2** (`retentionStats.js`) is **strictly additive** — the FSRS-based score formula was already research-backed (Karpicke & Roediger 2008 cited in `schema.prisma:955-958`) and is preserved verbatim. v2 adds three things: (1) **leech detection** via `lapseCount ≥ 8` (Anki convention from `schema.prisma:817-818`) — surfaced in `basis` lines and as a tier gate; (2) **tier mastery gates** with sample-size minimums (Lange, Wang, Dunlosky 2013): tier3=`retentionAttempts ≥ 5`, tier2=`≥10 attempts + retentionScore ≥ 60`, FAANG=`≥25 attempts + score ≥ 75 + retentionLeechRate ≤ 0.20`; (3) **Verdict Rule 13** hard-rejecting high-confidence retention strengths with n<10 (forces small-sample hedging in prose). The `retentionLeechRate ≤ 0.20` gate uses **inverse comparison** in `classifyReadiness` — see the `isMaxKey` branch at `readinessTiers.js`; when a third such max-threshold key appears, generalize the list. Independent flag pairing: `FEATURE_RETENTION_V2` + `VITE_FEATURE_RETENTION_V2`.

The merged `masteryCounts` object passed to `classifyReadiness` aggregates D1, D2, D3, D4, D6 keys into one bag with **non-overlapping prefixes**: `core*` / `owned` / `solidOrAbove` / `workingOrAbove` (patterns), `solutionsAt*` (solution depth), `comm*` (communication source quality), `opt*` (optimization trade-off), `retention*` (knowledge retention). D5 v2 does NOT add mastery keys — its source-tier ceiling enforces tier gates via the existing score threshold. When adding a new dimension's tier gates, pick a prefix that won't collide.
