...
// Non-color signal for each verdict so red-green colorblind users get the
// same information density. Shape prefix inside the badge; aria-hidden
// because the text label carries the semantic meaning.
const VERDICT_ICON = {
  STRONG:   CheckCircle2,
  ADEQUATE: Minus,
  WEAK:     AlertTriangle,
...
export default function CodeReviewResult({ review, usedFallback = false }) {
  ...
  return (
    <div className="space-y-6">
      {/* Fallback banner — verdict was generated by the deterministic
          structural check, not the AI. Learner should retry for a real
          review; leaving this un-badged permanently blocks reveal when
          the AI transiently failed. */}
      {usedFallback && (
        <div className="rounded-lg border border-warning-line bg-warning-soft p-3 flex items-start gap-2">
          ...
          <div className="flex-1">
            <p className="text-xs font-bold uppercase tracking-wider text-warning-fg">
              AI reviewer wasn't available
            </p>
            <p className="text-xs text-warning-fg leading-relaxed mt-0.5">
              The verdict below is from a conservative structural check.
              Resubmit for a full AI review — your code is preserved.
            </p>
          </div>
        </div>
      )}

      {/* topFocus — the AI's single most-actionable output. Rendered at the
          top so it's the first thing the learner sees after a WEAK/ADEQUATE
          verdict; it's what they'll actually change on the next attempt. */}
      {review.topFocus && (
        <section className="rounded-xl border border-brand-line bg-brand-soft p-4">
          <h4 className="text-[10px] font-bold uppercase tracking-widest text-brand-fg-soft mb-1">
            Focus on this
          </h4>
