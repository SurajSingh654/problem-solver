...
export const ROADMAP_ITEMS = [
    ...
    {
        id: 'review-queue-concept-repair-track',
        phase: 'NEXT',
        theme: 'Learning Science',
        priority: 'HIGH',
        effort: 'Large',
        title: 'Review Queue — parallel concept-repair track (Option B, brainstorm parked 2026-07-15)',
        impact: 'Current SM-2 review flow rewards memorization, not mastery. When a learner rates 1-2 (Forgot / Very Hazy), the system just shortens the interval and shows the SAME problem sooner — the learner recognizes their own code the next day and rates 4, but couldn\'t solve a NEW problem on the same pattern. Bjork\'s illusion-of-fluency in action. PO named this on 2026-07-14 during Rule 24 debrief. The redesign adds a parallel concept-repair track: on low-rating, open a diagnostic session that identifies the specific gap (pattern / recurrence / base case / etc.), presents a similar-difficulty drill from a curated pool, walks the learner through worked-example fading + teach-back if the drill fails, then requires them to retry the ORIGINAL problem cold before it re-enters SM-2. SM-2 stays untouched for the 80% "real memory decay on solid concepts" case; concept-repair handles the 20% "schema is actually broken" case.',
        description: 'Brainstorm executed 2026-07-15 through superpowers:brainstorming skill — 4 parallel research agents (learning-science, competitive, PO audit, behavioral-data), all four converged on the diagnosis. Design walked through architecture-shape screen + plain-English before/after + cascade/boundary analysis + session-mechanics state machine. Locked at Section 3 threshold. Full locked-vs-pending catalog:\n\n' +
            '═══ LOCKED ═══\n' +
            '1. ARCHITECTURE — parallel concept-repair queue (Option B). SM-2 unchanged for retention. New queue handles pattern-weakness. Both queues coexist, never fight.\n' +
            '2. PHILOSOPHY — same-problem retry. Repair diagnoses THIS problem\'s root cause and retries THE SAME problem after fixing it. Drill is a similar-difficulty problem for transfer verification, not a replacement target.\n' +
            '3. CONTENT-TYPE BOUNDARY — three NEW tables strictly separate from Problem / Solution: RepairDrill (curated pool), RepairEntry (per user, per Solution), RepairAttempt (drill attempts + retry-original attempts). Drills NEVER enter Problems section, NEVER create Solution rows, NEVER enter SM-2 (except at COMPLETED — see #13).\n' +
            '4. CONTENT SOURCING (Option A) — curated RepairDrill pool authored by content team. MVP seed: ~90 drills (30 most-common patterns × 3 difficulty tiers × 1 drill each). Full scale: ~180 drills. AI-generated (Option C) explicitly rejected by user ("not compromising on anything"). Reused untouched Problems (Option B) rejected for UX-clarity reasons.\n' +
            '5. SESSION-SCOPE BOUNDARY — one active RepairEntry per Solution (idempotent — retriggering a no-op). Failed drill = escalate DEPTH in same session, NEVER spawn new drill on new problem.\n' +
            '6. MVP TRIGGERS (per-problem, immediate) — Trigger 1: rating 1-2 on a review; Trigger 3: AI wrongPattern OR patternAccuracy < 4/10 on initial solve. Both create the same RepairEntry targeting one Solution.\n' +
            '7. DROPPED TRIGGER — pattern-lapse accumulation (doesn\'t fit per-problem model — user gets 3 repairs for 3 failed DP problems, not one aggregate pattern repair).\n' +
            '8. DEFERRED TRIGGERS — Trigger 4 (confidence miscalibration; needs recallQualityAiScore schema addition per behavioral-signal audit); Trigger 5 (learner opt-in; low-priority self-service).\n' +
            '9. SESSION FLOW · 5 STAGES — (1) Open: trigger fires → RepairEntry created. (2) Diagnostic: user picks fuzzy parts, optional AI-inferred hint from recallText. (3) Drill: pull from curated pool matching pattern + difficulty, AI-graded. (4) Retry Original: user solves the ORIGINAL problem cold. (5) Close: COMPLETED / ABANDONED / TIMED_OUT.\n' +
            '10. ESCALATION LADDER · 5 RUNGS (depth not breadth) — Rung 1 default drill on its own. Rung 2 + worked-example fading (Renkl 2004). Rung 3 + teach-back prompt (Feynman-style, AI-graded). Rung 4 + progressive hints (hint-assisted pass counts as learning signal but does NOT unlock retry-original). Rung 5 pause / manual escape ("come back tomorrow with fresh eyes" or link to curriculum concept if one exists).\n' +
            '11. EXIT CONDITIONS · 3 STATES — COMPLETED: user solved original cold + confident ≥ 4 → Solution SM-2 resets to fresh baseline, ConceptMastery signal fires. ABANDONED: user explicit skip → Solution SM-2 untouched. TIMED_OUT: 14 days idle → auto-abandoned.\n' +
            '12. DRILL PASS SIGNAL (Option C — strictest gate) — AI-graded (passes drill\'s canonical evaluation) AND user confidence ≥ 4/5. BOTH required. Prevents user gaming and AI false-negatives simultaneously. Hint-assisted drill pass doesn\'t unlock Stage 4 retry-original — Rungs 1-3 clean pass required for that gate.\n' +
            '13. SM-2 HANDLING — Solution\'s SM-2 state UNTOUCHED during repair (nextReviewDate stays where it was pre-trigger). Only on COMPLETED does SM-2 reset: sm2EasinessFactor=2.5, sm2Interval=1, sm2Repetitions=0, nextReviewDate=tomorrow, lapseCount unchanged (historical audit preserved), reviewCount unchanged.\n\n' +
            '═══ PENDING ═══\n' +
            'A. SCHEMA DETAILS (Section 3, not walked) — exact field lists for the 3 new tables, indexes, migrations, optional ReviewAttempt.repairEntryId FK for audit, feature flag FEATURE_CONCEPT_REPAIR_QUEUE + VITE mirror.\n' +
            'B. UI SURFACES (Section 4, not walked) — sidebar badge count/color for active RepairEntries, repair-queue page layout (distinct from review-queue page), repair-session shell UI (diagnostic checkboxes + drill editor + rung state indicator + retry-original gate), TEAM_ADMIN content-authoring surface for RepairDrill CRUD.\n' +
            'C. AI-GRADING PIPELINE (Section 5, partial) — does drill AI grading route through existing solutionReviewPrompt / validateReview, or a NEW dedicated CODE_DRILL_REVIEW validator (parallel to CODE_REVIEW / CODE_WALKTHROUGH)? Rule N gate for coherence?\n' +
            'D. INTEGRATION (Section 6, not walked) — ConceptMastery signal source addition (concept_repair_completed); D6 leech metric redefinition (current: lapseCount≥8; new: consider "in repair" as a distinct state); D10 metacognition — feed the session\'s user-confidence-vs-AI-grade delta into calibrationDelta; patternMastery matrix — does repair completion advance UNTOUCHED→TOUCHED→WORKING transitions; Curriculum ConceptMastery cross-link if Solution\'s pattern maps to a Curriculum Concept.\n' +
            'E. ROLLOUT + MIGRATION (Section 7, not walked) — feature flag defaults, kill switch; existing Solutions with lapseCount≥8 on flag flip (auto-enroll or wait-for-next-lapse?); content debt scheduling for the ~90 drill MVP seed; success metrics post-flip (repair-completion rate, retry-original cold-solve rate, transfer test on subsequent same-pattern problems); rollback semantics (mid-repair sessions when flag flips OFF).\n' +
            'F. OPEN UX QUESTIONS from mid-brainstorm — Stage 2 AI-inferred diagnostic hint (AI-driven from recallText vs user-picked dropdown for MVP); Rung 5 curriculum-link (only fires if concept exists — how to check).\n' +
            'G. TEST PLAN (not scoped) — integration tests for RepairEntry state machine transitions, prompt-injection defense on drill attempts (user code → AI grading), tenancy (RepairEntry team-scoped through Solution → Problem → team), feature-flag OFF path (route 404s), race test (two concurrent triggers on same Solution → idempotent single RepairEntry).\n\n' +
            'When resuming: continue the superpowers:brainstorming skill from Section 3 (data model). Session state persisted in .superpowers/brainstorm/ (gitignored). All research agent outputs captured in the brainstorm session artifacts.',
        why: 'Real product-quality issue observed in production, not a hypothetical. Current SM-2 optimizes retrieval probability at target interval (declarative-memory metric from Wozniak 1990) — validated on paired-associates and vocabulary, unvalidated for procedural knowledge (Kang 2016). Same-problem reschedule cannot fix a broken schema (Gick & Holyoak 1983 — schema induction requires multiple analogs, the OPPOSITE of what SM-2 does). PO named the memorization symptom directly during Rule 24 debrief. Only industry precedent for a "concept broken" state distinct from "memory faded" is Anki\'s leech-suspend — nobody publicly routes a failed review to a different-but-similar problem on the same pattern. Design is differentiating, not table-stakes. Cost: substantial — schema + UI + AI validator + content authoring. Benefit: closes the memorization-vs-mastery gap that today\'s D6/D10 metrics report but don\'t remediate.',
        technicalNotes: 'Grounded on the current codebase per PO-audit + behavioral-signal agents (2026-07-15). Key existing surfaces: server/prisma/schema.prisma Solution model (lines 761-919) — SM-2 fields already present; ReviewAttempt table (lines 1005-1029) — audit trail. server/src/utils/sm2.js — SM-2 algorithm (177 lines). server/src/controllers/solutions.controller.js submitReview (275-394) — where trigger interception lands. server/src/utils/patternMastery.js — 5-state matrix already exists (UNTOUCHED/TOUCHED/WORKING/SOLID/OWNED). server/src/utils/retentionStats.js — D6 leech definition (lapseCount ≥ 8, line 50-53); D10 verificationStats.js — calibrationDelta (lines 165-387). client/src/pages/ReviewQueuePage.jsx — 3-phase review UI (recall → reveal → rate); AI recall-grade result is currently ephemeral (never persisted — noted as biggest wasted signal by PO agent). Persisting recallQualityAiScore is a prerequisite for Trigger 4 (deferred). Small schema additions (behavioral-signal agent): ReviewAttempt.recallTimeMs (Int?), ReviewAttempt.codeChangedFromOriginal (Boolean via server-side hash compare), ReviewAttempt.recallQualityAiScore (Int? 1-5). GIN index on Solution.patterns not needed for MVP triggers. Brainstorm artifacts (visual screens, agent reports, session state) persist at .superpowers/brainstorm/ — restart via scripts/start-server.sh when resuming.',
    },
