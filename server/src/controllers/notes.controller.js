...
// ============================================================================
// DUPLICATE — create a new note from an existing one
// ============================================================================
//
// Use case: a user keeps a "Templates" (or any reference) folder and wants
// to spin up a fresh note pre-filled with the contents of one of those
// notes. This is generic — there is no template concept on the server.
// Any note the user owns can be duplicated.
//
// Copied:   title (prefixed "Copy of …"), contentMarkdown, tags, folderId
// Reset:    pinned (false), archivedAt (null), summary (null), suggestedTags
//           (empty), linkedEntity* (cleared — those tied the source note
//           to a specific Problem/Session, the copy starts fresh),
//           autoGenerated (false), embedding (regenerated async)
//
// The copy lands in the same folder as the source by default — most users
// duplicating a "template" want the new note next to the original; if
// they want it elsewhere, the new MoveToFolderMenu is one click.
// ============================================================================
export async function duplicateNote(req, res) {
  try {
    ...
    if (!source) return error(res, "Note not found", 404);

    const copyTitle = trimTitle(`Copy of ${source.title}`);
    ...
    console.error("duplicateNote:", err);
    return error(res, "Failed to duplicate note", 500);
  }
}
