...
function parseProblem(p) {
  return {
    ...
    aiRealWorldSuggestions: JSON.parse(
      p.aiRealWorldSuggestions || '[]'
    ),
  }
}
...
export async function getProblems(req, res) {
  ...
  let problems = await prisma.problem.findMany({
    ...
    include: {
      ...
      followUps: { select: { id: true, question: true, difficulty: true, hint: true, order: true }, orderBy: { order: 'asc' } },
...
export async function getProblemById(req, res) {
  ...
  const problem = await prisma.problem.findUnique({
    ...
    include: {
      ...
      solutions: {
        include: {
          user          : { select: { username: true, avatarColor: true, currentLevel: true } },
          clarityRatings: { include: { fromUser: { select: { username: true } } } },
        },
...
export async function updateProblem(req, res) {
  ...
  const updated = await prisma.problem.update({
    ...
    data : {
      ...
      ...(relatedProblems  !== undefined && { relatedProblems: JSON.stringify(relatedProblems) }),
    },
    include: {
      addedBy  : { select: { username: true } },
      followUps: true,
