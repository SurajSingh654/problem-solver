...
import { sendTeamInviteEmail, sendTeamApprovedEmail, sendTeamRejectedEmail } from '../services/email.service.js'
...
export async function inviteMembers(req, res) {
  try {
    ...
    for (const email of emails) {
      ...
      // ── Send email ───────────────────────────────────
      sendTeamInviteEmail(email, team.name, team.joinCode, invitation.token).catch((err) => {
