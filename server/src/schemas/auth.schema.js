...
export const requestEmailChangeSchema = z.object({
  newEmail: emailField,
  password: z.string({ required_error: 'Password is required for email changes.' }).min(1),
})
...
export const onboardingSchema = z.object({
  ...
  {
    message: 'Please provide a join code to join a team or a name to create one.',
    path: ['joinCode'],
  }
)
...
export const updateProfileSchema = z.object({
  ...
  targetCompany: z.string().max(100).transform((v) => v.trim()).optional().nullable(),
  ...
  aiProblemConfig: z.object({
    categories: z.array(z.enum(['CODING', 'SYSTEM_DESIGN', 'BEHAVIORAL', 'CS_FUNDAMENTALS', 'HR', 'SQL'])).optional(),
